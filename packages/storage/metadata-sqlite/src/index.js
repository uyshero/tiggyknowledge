...
export class MetadataSqlite extends Service {
    ...
    schemaVersion() {
        const row = this.requireDatabase().prepare("SELECT value FROM metadata WHERE key = 'metadata_schema_version'").get();
    ...
    renameTag(tagId, name) {
        const database = this.requireDatabase();
        const current = database.prepare('SELECT id, name, 0 AS document_count, created_at, updated_at FROM tags WHERE id = ?').get(tagId);
        if (current === undefined)
            throw new RangeError('标签不存在');
        const nextName = name.trim();
        if (nextName.length === 0 || nextName.length > 32)
            throw new RangeError('标签名称长度应为 1 到 32 个字符');
        const existing = database.prepare('SELECT id, name, 0 AS document_count, created_at, updated_at FROM tags WHERE name = ? COLLATE NOCASE').get(nextName);
        const now = new Date().toISOString();
        database.exec('BEGIN IMMEDIATE');
        try {
      ...
      ORDER BY tags.name COLLATE NOCASE
      LIMIT 1
    `).get(current.id, nextName);
        if (row === undefined)
            throw new Error('metadata-sqlite: tag rename failed');
    ...
    taggedDocumentIds(tagId) {
        const rows = this.requireDatabase().prepare('SELECT document_id FROM document_tags WHERE tag_id = ? ORDER BY created_at DESC').all(tagId);
        return rows.map(row => row.document_id);
    }
    favoriteDocumentIds() {
        const rows = this.requireDatabase().prepare('SELECT document_id FROM document_favorites ORDER BY created_at DESC').all();
        return rows.map(row => row.document_id);
    }
    requireDatabase() {
        if (this.database === undefined)
            throw new Error('metadata-sqlite: database is not initialized');

...
function neighborhood(documentId, depth, outgoing, incoming) {
    const result = new Set([documentId]);
    let frontier = new Set([documentId]);
    for (let level = 0; level < depth; level += 1) {
        const next = new Set();
        for (const current of frontier) {
            const neighbors = new Set([...(outgoing.get(current) ?? []), ...(incoming.get(current) ?? [])]);
...
export class KnowledgeGraph extends Service {
    ...
    async snapshot(query = {}) {
        ...
        if (documentId !== undefined) {
            return this.document(documentId, { depth, includeMissing, libraryId });
        }
    ...
    async document(documentId, query = {}) {
        const state = await this.ensureState();
        ...
        const nodes = [...scopeIds].map(id => state.nodesById.get(id)).filter((node) => node !== undefined);
        const missingNodes = includeMissing
            ? [...new Set(edges.map(edge => edge.target).filter(id => id.startsWith('missing:')))]
                .map(id => state.nodesById.get(id))
                .filter((node) => node !== undefined)
    ...
    async rebuild() {
        ...
        for (const record of records) {
            for (const key of documentKeys(record.document)) {
                ...
                if (list === undefined)
                    keys.set(key, [record.document.id]);
                else if (!list.includes(record.document.id))
                    list.push(record.document.id);
            }
        }
        ...
        const resolveDocument = (source, target) => {
            ...
            for (const key of candidates) {
                ...
                if (sameLibrary !== undefined)
                    return records.find(record => record.document.id === sameLibrary)?.document;
            }
            for (const key of candidates) {
                const matches = keys.get(key) ?? [];
                if (matches[0] !== undefined)
                    return records.find(record => record.document.id === matches[0])?.document;
            }
            return undefined;
        };
        for (const record of records) {
            for (const raw of record.links) {
                ...
                if (resolved?.id === record.document.id)
                    continue;
                ...
                if (edgeIds.has(edgeId))
                    continue;
                edgeIds.add(edgeId);
                edges.push({ id: edgeId, source: record.document.id, target: targetId, label: resolved === undefined ? undefined : '链接' });
